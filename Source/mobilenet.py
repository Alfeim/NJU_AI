...
def conv2d(input_, output_dim, k_h, k_w, d_h, d_w, stddev=1.0, name='conv2d', bias=False):
...
def conv_1x1(input, output_dim, name, bias=False):
        return conv2d(input, output_dim, 1,1,1,1, stddev=0.005, name=name, bias=bias)
...
def dwise_conv(input, k_h=3, k_w=3, channel_multiplier= 1, strides=[1,1,1,1],
               padding='SAME', stddev=1.0, name='dwise_conv', bias=False):
    with tf.variable_scope(name):
...
def mobilenetv2(inputs, num_classes, is_train=True, reuse=False):
    ...
    logits = flatten(conv_1x1(net, num_classes, name='last'))
